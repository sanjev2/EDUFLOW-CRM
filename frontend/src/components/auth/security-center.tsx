...
type Identity = { user: { role: AppRole; mfaEnabled: boolean } };
...
export function SecurityCenter() {
  ...
  function applyIdentity(result: { sessions: Session[] }, me: Identity) {
  ...
  async function load() {
    await refreshCsrf();
    const result = await Promise.all([api<{ sessions: Session[] }>("/api/v1/sessions"), api<Identity>("/api/v1/auth/me")]);
  ...
  async function changePassword(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await api<{ message: string }>("/api/v1/auth/change-password", { method: "POST", body: JSON.stringify({ currentPassword: data.get("currentPassword"), password, passwordConfirmation: data.get("passwordConfirmation") }) });
  ...
  return <AppShell role={role} title="Security settings" subtitle="Manage your password, MFA and active sessions." actions={<button onClick={() => void logout()} className="rounded-lg border border-slate-300 bg-white px-4 py-2 font-semibold">Log out</button>}>
    <div className="mx-auto grid max-w-4xl gap-5">
      ...
      <Card><h2 className="text-xl font-bold">Change password</h2><form onSubmit={changePassword} className="mt-5 grid gap-4"><PasswordField label="Current password" name="currentPassword" autoComplete="current-password" required /><PasswordField label="New password" name="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" maxLength={128} required /><PasswordStrength password={password} /><PasswordField label="Confirm new password" name="passwordConfirmation" autoComplete="new-password" maxLength={128} required /><SubmitButton busy={busy}>Change password</SubmitButton></form></Card>
      <Card><h2 className="text-xl font-bold">Active sessions</h2><div className="mt-4 grid gap-3">{sessions.map((session) => <div key={session.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3"><div><p className="font-semibold">{session.current ? "This device" : session.userAgent}</p><p className="text-xs text-slate-500">Last active {new Date(session.lastActivityAt).toLocaleString()} · IP {session.ipAddress}</p></div>{!session.current && <button onClick={() => void revoke(session.id)} className="text-sm font-semibold text-red-700">Revoke</button>}</div>)}</div></Card>
    </div>
  </AppShell>;
}
