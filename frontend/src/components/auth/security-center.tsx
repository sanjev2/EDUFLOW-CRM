...
export function SecurityCenter() {
  ...
  async function load() {
    await refreshCsrf();
    const [result, me] = await Promise.all([api<{ sessions: Session[] }>("/api/v1/sessions"), api<{ user: { role: AppRole } }>("/api/v1/auth/me")]);
    setSessions(result.sessions); setRole(me.user.role);
  }
  useEffect(() => { void refreshCsrf().then(() => Promise.all([api<{ sessions: Session[] }>("/api/v1/sessions"), api<{ user: { role: AppRole } }>("/api/v1/auth/me")])).then(([result, me]) => { setSessions(result.sessions); setRole(me.user.role); }).catch((reason: Error) => setError(reason.message)); }, []);
