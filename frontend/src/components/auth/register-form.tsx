...
export function RegisterForm() {
  ...
  return <AuthShell title="Create your student account" description="Start securely with a verified email address.">{success ? <div className="mt-6 rounded-xl bg-emerald-50 p-4 text-emerald-900" role="status">Check the development outbox or your email for verification instructions.</div> : <form onSubmit={submit} className="mt-8 grid gap-5" noValidate><ErrorSummary message={error} /><Field label="Full name" name="fullName" autoComplete="name" required /><Field label="Email address" name="email" type="email" autoComplete="email" required /><PasswordField label="Password" name="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" maxLength={128} required /><PasswordStrength password={password} /><PasswordField label="Confirm password" name="passwordConfirmation" autoComplete="new-password" maxLength={128} required /><label className="flex gap-3 text-sm"><input name="consent" type="checkbox" required className="mt-1 h-4 w-4" /><span>I agree to the privacy notice and terms for using EduFlow.</span></label><SubmitButton busy={busy}>Create student account</SubmitButton></form>}<div className="mt-6 border-t border-[var(--border)] pt-5 text-sm text-slate-600"><p>Consultancy staff accounts are provided by an authorized administrator.</p><p className="mt-3">Already registered? <Link className="font-semibold text-blue-700" href="/login">Sign in</Link></p></div></AuthShell>;
}
